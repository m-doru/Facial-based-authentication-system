...
parser.add_argument(
    ...
    type=float,
    default=1,
    help="The scale with which the webcam frames will be " "resized",
)
parser.add_argument(
    "--webcam",
    type=int,
    default=0,
    help="Specify which camera to be used for input(0 = webcam, " "1 = usb",
)
...
def loadKnownFaces(path):
    ...
    if args.verbose:
        print(
            "Loading known faces representations took {}".format(
                time.time() - startTimeLoadingKnownFaces
            )
        )
...
def main():
    ...
    cameraFeed = cv2.VideoCapture(
        "/home/doru/Desktop/Licenta/Implementation/databases/cbsr_antispoofing/test_release"
        "/13/1.avi"
    )
    ...
    while True:
        ...
        for i, bb in enumerate(bbs):
            ...
            faceRep = net.forward(face)
            print(
                "Passing through the network takes {}".format(
                    time.time() - face_rep_start
                )
            )
        ...
        if args.verbose:
            print(
                "Processing a frame took {}".format(time.time() - startFrameProcessing)
            )
