...
def main(load_faces=False, load_distances=False, examples_per_subject=-1):
    ...
    saved_distances_same_id_filename = (
        "same_id_distances_" + str(examples_per_subject) + ".pkl"
    )
    saved_distances_diff_id_filename = (
        "diff_id_distances_" + str(examples_per_subject) + ".pkl"
    )

    if not load_distances or not (
        os.path.exists(saved_distances_same_id_filename)
        or os.path.exists(saved_distances_diff_id_filename)
    ):
        (same_id_distances, diff_id_distances) = get_faces_distances(
            faces, examples_per_subject
        )
    ...
    (n_d, _, _) = pyplot.hist(
        diff_id_distances, 40, normed=True, facecolor="red", alpha=0.75
    )
