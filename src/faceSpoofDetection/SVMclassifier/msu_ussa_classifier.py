...
def get_features_and_labels(load_train_features, mlbp_feature_computer, np_array=True):
    saved_realfaces_features_filename = (
        "../featuresVectors/msu_ussa_realfaces_features_joblib.pkl"
    )
    saved_spooffaces_features_filename = (
        "../featuresVectors/msu_ussa_spooffaces_features_joblib.pkl"
    )
    ...
    if load_train_features == False:
        ...
        spoof_features_per_dir = dbfeatures.compute_spoofface_features_msu_ussa(
            mlbp_feature_computer, [1, 2, 3, 4, 5]
        )
        joblib.dump(spoof_features_per_dir, saved_spooffaces_features_filename)

        print("Computing features for MSU USSA real faces")
        real_features = dbfeatures.compute_realface_features_msu_ussa(
            mlbp_feature_computer, [1, 2, 3, 4, 5]
        )
        ...
        if np_array:
            ...
            spoof_features_per_dir = np.asarray(
                [
                    np.asarray(spoof_features_per_dir[i])
    ...
    if load_train_features is not None:
        ...
        if np_array:
            ...
            labels_spoof_per_dir = np.asarray(
                [
                    np.asarray([-1 for _ in range(len(spoof_features_per_dir[i]))])
                    for i in range(len(spoof_features_per_dir))
                ]
            )
        else:
            ...
            labels_spoof_per_dir = [
                [-1 for _ in range(len(spoof_features_per_dir[i]))]
                for i in range(len(spoof_features_per_dir))
            ]
