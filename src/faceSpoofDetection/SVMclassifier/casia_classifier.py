...
def get_train_features_and_labels(load_train_features, mlbp_feature_computer):
    saved_realfaces_train_features_filename = (
        "../featuresVectors/casia_realfaces_features_" + version + extension
    )
    saved_spooffaces_train_features_filename = (
        "../featuresVectors/casia_spooffaces_features_" + version + extension
    )

    if load_train_features == False:
        # compute feature vectors for every frame in the videos with real faces
        real_features_train = dbfeatures.compute_realface_features_casia(
        ...
        spoof_features_train = dbfeatures.compute_spoofface_features_casia(
            mlbp_feature_computer
        )
...
def get_test_features_and_labels(load_test_features, mlbp_feature_computer):
    saved_realfaces_test_features_filename = (
        "../featuresVectors/casia_realfaces_test_features_" + version + extension
    )
    saved_spooffaces_test_features_filename = (
        "../featuresVectors/casia_spooffaces_test_features_" + version + extension
    )

    if not load_test_features:
        real_features_test = dbfeatures.compute_realface_features_casia(
            mlbp_feature_computer, train=False
        )
        print("Computed real test features")
        joblib.dump(real_features_test, saved_realfaces_test_features_filename)
        print("Saved real test features")

        spoof_features_test = dbfeatures.compute_spoofface_features_casia(
            mlbp_feature_computer, train=False
        )
        print("Computed spoof test features")
        joblib.dump(spoof_features_test, saved_spooffaces_test_features_filename)
        print("Saved spoof test features")
    else:
        real_features_test = joblib.load(saved_realfaces_test_features_filename)
        print("Loaded real test features")
        spoof_features_test = joblib.load(saved_spooffaces_test_features_filename)
        print("Loaded spoof test features")
...
def main():
    ...
    if not load_classifier:
        ...
        param_grid = {'C':[0.0001, 0.01,1], 'kernel':['rbf'], 'class_weight':['balanced', None], 'gamma':[0.0001,
                                                                                                             0.1, 10]}
        """
        param_grid = {
            "C": [0.000001, 0.01, 1, 10, 100],
            "kernel": ["linear"],
        ...
        # clf = svm.SVC(verbose=True, probability=True, C = 0.01, kernel='rbf', gamma=0.1, class_weight='balanced')
        ...
        joblib.dump(clf, saved_classifier_filename + "linear")
    else:
