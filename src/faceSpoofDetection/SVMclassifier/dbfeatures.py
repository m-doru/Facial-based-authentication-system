...
def compute_face_features_idiap(
    feature_computer, stage="train", real=True, verbose=False
):
    ...
    if verbose:
        print(
            "Started processing idiap {} faces database for {}".format(face_type, stage)
        )
        ...
        for dir in dirs:
            ...
            for _, _, files in os.walk(path):
                for file in files:
                    ...
                    video_features = get_frames_features(
                        full_file_path, feature_computer
                    )
...
def get_msu_mfsd_stage_subjects(train=True):
    if train:
        sub_list_filename = os.path.join(
            dbsDir, "MSU_MFSD", "MSU-MFSD", "train_sub_list.txt"
        )
    else:
        sub_list_filename = os.path.join(
            dbsDir, "MSU_MFSD", "MSU-MFSD", "test_sub_list.txt"
        )
...
def compute_spoofface_features_msu_ussa(
    feature_computer,
    five_fold_train,
    dirs=(
        "MacBook_FrontCamera",
        ...
        for _, _, files in os.walk(dirpath):
            ...
            for file in files:
                ...
                if fold_number in five_fold_train:
                    ...
                    image_features = get_frames_features(
                        full_file_path, feature_computer
                    )
                    features.extend(image_features)
            break
